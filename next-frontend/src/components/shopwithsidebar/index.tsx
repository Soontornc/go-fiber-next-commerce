...
const ShopWithSidebar = () => {
  ...
  const { products, isLoading, error, pagination, fetchProducts } = useProduct();
  ...
  useEffect(() => {
    fetchProducts(currentPage, 10, 'created_at', 'desc')
  ...
  useEffect(() => {
    ...
    function handleClickOutside(event: MouseEvent) {
      if (
        !event.target ||
        !(event.target as Element).closest(".sidebar-content")
      ) {
              ...
              <div>
                {isLoading ? (
                  ...
                  <>
                    {/* <!-- product grid view --> */}
                    <div
                      className={`grid-cols-3 sm:grid-cols-2 lg:grid-cols-3 gap-x-7.5 gap-y-10 ${
                        productStyle === "grid" ? "grid" : "hidden"
                      }`}
                    >
                      {products.map((item) => (
                    ...
                    <div
                      className={`flex flex-col gap-10 ${
                        productStyle === "list" ? "block" : "hidden"
                      }`}
                    >
                      {products.map((item) => (
